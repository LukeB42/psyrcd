...
def nsmsg_list(t):
    for r in t:
        ...
        if user:
            if 'R' in user.modes:
                nsmsg("\x02\x033%s\x0F:%s Ident: %s, Registered: %s" % \
				(r['nick'], ip, r['ident'], fmt_timestamp(r['time_reg'])))
            else:
                nsmsg("\x02\x032%s\x0F:%s Ident: %s, Registered: %s" % \
				(r['nick'], ip, r['ident'], fmt_timestamp(r['time_reg'])))
        else:
            nsmsg("\x02%s\x0F:%s Ident: %s, Registered: %s" % \
			(r['nick'], ip, r['ident'], fmt_timestamp(r['time_reg'])))
...
if 'init' in dir():
    provides = ['command:nickserv,ns:Nickname registration service.',
                'umode:R:Registered nickname.',
    ...
    if init:
        ...
        if not 'db' in cache:
            ...
            cache['db'] = db
            db.execute(
                "CREATE TABLE IF NOT EXISTS %s (ip, ident, nick, password, time_reg REAL, time_use REAL)"
                % TABLE)
            db.commit()
    else:
        if 'db' in cache:
...
if 'func' in dir():
    if func.func_name == 'handle_join':
        if 'channel' in dir():
            if 'c' in channel.modes and not 'R' in client.modes:
                nsmsg("A registered nick is required to join %s." %
                      channel.name)
                params = ''
    ...
    if func.func_name == 'handle_nick':
        db = cache['db']
        c = db.cursor()
        c.execute("SELECT * FROM %s WHERE nick='%s'" % \
			(TABLE, escape(params)))
        r = c.fetchone()
...
if 'new' in dir() and 'channel' not in dir():
    ...
    if client.nick:
        ...
        c = db.cursor()
        c.execute("SELECT * FROM %s WHERE nick='%s'" % \
			(TABLE, escape(client.nick)))
        r = c.fetchone()
        if r: nsmsg("This nickname is owned by \x02%s\x0F." % r['ident'])
        else:
            nsmsg(
                "Use \x02/NICKSERV HELP REGISTER\x0F for information on registering this nick.")
            if WAIT_MINUTES:
                nsmsg("You must be connected for at least %i minutes before you can register."
                      % WAIT_MINUTES)
        del c, r
...
if 'command' in dir():
    ...
    if cmd == 'help' or not cmd:
        if not args:
            ...
            nsmsg("Up to \x02%i\x0F nicknames may be registered per IP address."
                  % MAX_NICKS)
            nsmsg("Nicknames will be forgotten about after %i days if not used."
                  % MAX_DAYS_UNPRESENT)
        ...
        if 'R' in client.modes:
            ...
            if not args:
                nsmsg("Usage: \x02/NICKSERV REGISTER \x1Fpassword\x0F")
                ...
                r = c.fetchone()
                if r:
                    nsmsg("Nickname \x02%s\x0F is already registered." %
                          escape(client.nick))
                else:
                    c.execute("SELECT * FROM %s WHERE ip='%s'" % \
						(TABLE, client.host[0]))
                    r = c.fetchall()
                    if len(r) >= MAX_NICKS:
                        nsmsg("You already have %i nicknames registered to this ip address:"
                              % MAX_NICKS)
                        for i in r:
                            nsmsg("\x02%s\x0F, %s" %
                                  (i['nick'], fmt_timestamp(i['time_reg'])))
                        del i
                    else:
                        t = time.time()
                        ...
                        db.commit()
                        nsmsg("Registered \x02%s\x0F to \x02%s\x0F." %
        ...
        if 'R' in client.modes:
            ...
            if not args:
                nsmsg("Usage: \x02/NICKSERV IDENTIFY \x1Fpassword\x0F")
            else:
                ...
                if not r: nsmsg("Your nick isn't registered.")
                    ...
                    if is_expired(r['time_use']):
                        c.execute("DELETE FROM %s WHERE nick='%s'" % \
							(TABLE, escape(client.nick)))
                        db.commit()
                        nsmsg("\x02%s\x0F has expired due to inactivity." %
                              nick)
                        client.broadcast('umode:W', ':%s NOTICE * :\x02%s\x0F has expired.' % \
							(NS_IDENT, client.nick, nick))
                    elif r['password'] == password:
                        c.execute("UPDATE %s SET time_use = %f WHERE nick='%s'" % \
							(TABLE, time.time(), escape(client.nick)))
                        db.commit()
                        if 'R' in client.supported_modes:
                            client.modes['R'] = 1
                            nsmsg("Authentication successful for \x02%s\x0F." %
                                  client.nick)
                            client.broadcast(client.nick, ':%s MODE %s +R' %
                                             (NS_IDENT, client.nick))
                    else:
        ...
        # Opers: /ns password target-nick new-password
            ...
            if not r: nsmsg("\x02%s\x0F isn't registered." % nick)
                ...
                if not 'R' in client.modes:
                    ...
                    nsmsg(
                        "Contact an IRC Operator for help retrieving accounts.")
                    client.broadcast(
                        'umode:W',
                        ':%s NOTICE * :\x02%s\x0F needs help resetting their password.'
                        % (NS_IDENT, client.nick))
                    ...
                    c.execute("UPDATE %s SET password='%s' WHERE nick='%s'" % \
						(TABLE, password, escape(client.nick)))
                    nsmsg("Changed password for \x02%s\x0F." %
                          escape(client.nick))
            del db, c, r
        ...
        if not args or not ' ' in args:
            nsmsg(
                "Usage: \x02/NICKSERV GHOST \x1Fnick\x0F \x02\x1Fpassword\x0F")
        else:
            nick, password = args.split()
            password = hashlib.sha1(password).hexdigest()
            ...
            if not user: nsmsg("Unknown nick.")
                ...
                if not r: nsmsg("\x02%s\x0F isn't registered." % client.nick)
                    ...
                    if r['password'] != password:
                        ...
                        user.finish(':%s QUIT :GHOST command used by %s.' %
                                    (user.client_ident(True), client.nick))
        ...
        if not args: nsmsg("Usage \x02/NICKSERV LIST \x1Fpattern\x0F")
            ...
            if not client.oper and 'R' not in client.modes:
                ...
                db = cache['db']
                c = db.cursor()
                c.execute("SELECT * FROM %s WHERE ident LIKE '%s'" %
                          (TABLE, args))
                t = c.fetchall()
                nsmsg_list(t)
                del db, c, t
        ...
        if not args: nmsg("Usage: \x02/NICKSERV INFO \x1FNICK\x0F")
            ...
            if not r: nsmsg("\x02%s\x0F isn't registered." % args)
                ...
                user = client.server.clients.get(r['nick'])
                if user:
                    nsmsg(" Is online from: %s" %
                          user.client_ident(True).split("!")[1])
        ...
        if not args or not ' ' in args:
            nsmsg("Usage: \x02/NICKSERV DROP \x1Fnick\x0F \x02\x1Fpassword\x0F")
        else:
            nick, password = args.split()
            password = hashlib.sha1(password).hexdigest()
            db = cache['db']
            ...
            if not r: nsmsg("\x02%s\x0F isn't registered." % nick)
                ...
                if client.oper or (r['password'] == password):
                    ...
                    if user and 'R' in user.modes:
                        del user.modes['R']
                        client.broadcast(user.nick, ":%s MODE %s -R" %
                                         (NS_IDENT, client.nick))
                else:
                    nsmsg("Incorrect password.")
        ...
        if not client.oper:
            ...
            nsmsg(
                "Use \x02/NICKSERV HELP\x0F for a list of available commands.")
        else:
            db = cache['db']
            c = db.cursor()
            ...
            for r in t:
                if is_expired(r['time_use']):
                    ...
                    nsmsg("\x02%s\x0F has expired due to inactivity." %
                          r['nick'])
                    client.broadcast('umode:W',
                                     ':%s NOTICE * :%s expired \x02%s\x0F.' %
                                     (NS_IDENT, client.nick, r['nick']))
            db.commit()
