...
class Channel(object):
    ...
    def __init__(self, channel):
        ...
        self.c.execute("SELECT * FROM %s WHERE channel='%s'" % \
			(TABLE, self.channel))
            ...
            if ':' in key:
                ...
                if type(o) == dict:
                    ...
                    v = str(o)\
						...
						.replace(' ', '')\
						.replace("'", '')
                    self.c.execute("UPDATE %s SET %s='%s' WHERE channel='%s'" % \
    ...
    def __delitem__(self, key):
        if ':' in key:
            ...
            self.c.execute("DELETE FROM %s WHERE channel='%s'" %
                           (TABLE, self.channel))
        else:
            self[key] = ''
    ...
    def __repr__(self):
        if self.r:
            return ("<Channel object for %s at %s>" % \
...
def init_channel(client, channel):
    ...
    if c.r:
        ...
        if 'R' in client.modes and (client.nick == c['owner'] or client.nick ==
                                    c['successor']):
    ...
    for x in mode:
        ...
        if channel and m in channel.supported_modes:
            ...
            if f == '+' and not args in channel.modes[m]:
                ...
                if args and args in channel.modes[m]:
                    channel.modes[m].remove(args)
                else:
                    del channel.modes[m]
            if not args:
                client.broadcast(channel.name, ':%s MODE %s %s' %
                                 (CS_IDENT, channel.name, f + m))
            else:
...
def csmsg_list(t):
    for r in t:
        ...
        if chan:
            if 'R' in chan.modes:
                csmsg("\x02\x033%s\x0F:%s Description: %s, Registered: %s" % \
				(r['channel'], ip, r['description'], fmt_timestamp(r['time_reg'])))
            else:
                csmsg("\x02\x032%s\x0F:%s Description: %s, Registered: %s" % \
				(r['channel'], ip, r['description'], fmt_timestamp(r['time_reg'])))
        else:
            csmsg("\x02%s\x0F:%s Description: %s, Registered: %s" % \
			(r['channel'], ip, r['description'], fmt_timestamp(r['time_reg'])))
...
class CSError(Exception):
    def __init__(self, value):
        self.value = value  # causes error messages to be

    def __str__(self):
        return (repr(self.value))  # dispersed to umode:W users


if 'init' in dir():
    provides = ['command:chanserv,cs:Channel registration service.',
                'cmode:R:Registered channel.']
    if init:
        ...
        if not 'db' in cache:
            ...
            cache['db'] = db
            db.execute(
                "CREATE TABLE IF NOT EXISTS %s (channel, password, description, \
    ...
    if c.r:
        ...
        if func.func_name == 'handle_join':
            ...
            if params.split()[1] == c['owner']:
                ...
                if not client.oper and (client.nick != c['owner'] or 'R' not in
                                        client.modes):
            ...
            # SecureOps / Peace
                ...
                if mode[1:] in ['v', 'h', 'o', 'a', 'q']:
                    ...
                    elif (c['peace'] and mode[0] == '-') and (
                        'R' not in client.modes or client.nick != c['owner']):
...
if 'command' in dir():
    ...
    if cmd == 'help' or not cmd:
        if not args:
            csmsg(
                "\x02/CHANSERV\x0F allows you to register and control various aspects of")
            csmsg(
                "channels. ChanServ can often prevent malicious users from \"taking")
            csmsg("over\" channels by limiting who is allowed channel operator")
            csmsg(
                "privileges. Available commands are listed below; to use them, type")
            csmsg(
                "\x02/CHANSERV \x1Fcommand\x0F. For more information on a specific command,")
            ...
            csmsg(
                "Syntax: \x02REGISTER \x1Fchannel\x0F \x02\x1Fpassword\x0F \x02\x1Fdescription\x0F")
            csmsg("")
        ...
        if args == 'set founder':
            ...
            csmsg("")
            csmsg(
                "When \x02peace\x0F is set, a user won't be able to kick, ban")
            ...
            csmsg("")
            csmsg(
                "Maintains the \x02SOP\x0F (SUperOp) \x02list\x0F for a channel.")
            csmsg("")
            ...
            csmsg(
                "Syntax \x02KICK \x1Fchannel\x0F \x02\x1Fnick\x0F \x02\x1Freason\x0F")
            csmsg("")
        ...
        if not args or len(args.split()) < 3:
            ...
            if not re.match('^#([a-zA-Z0-9_])+$', channel_name):
                ...
                db = cache['db']
                c = db.cursor()
                c.execute("SELECT * FROM %s WHERE channel='%s'" % \
					(TABLE, escape(channel_name)))
                r = c.fetchone()
                if r: csmsg("\x02%s\x0F is already registered." % channel_name)
                else:
                    c.execute("SELECT * FROM %s WHERE owner='%s'" % \
						(TABLE, client.nick))
                    r = c.fetchall()
                    if len(r) >= MAX_CHANNELS:
                        csmsg("You already have %i channels registered to this nick:"
                              % MAX_CHANNELS)
                        for i in r:
                            csmsg("\x02%s\x0F, %s" %
                                  (i['channel'], fmt_timestamp(i['time_reg'])))
                        del i
                    else:
                ...
                if not 'A' in client.modes and client.nick != c['owner'] and client.nick != c['successor']:
                    ...
                    if option == 'founder':
                        if not 'A' in client.modes and client.nick != c['owner']:
                            ...
                            cur = db.cursor()
                            cur.execute("SELECT * FROM %s WHERE nick='%s'" %
                                        (NS_TABLE, escape(params)))
                            r = cur.fetchone()
                            if not r:
                                csmsg("\x02%s\x0F isn't a registered nick." %
                                      params)
                            else:
                                c['owner'] = escape(params)
                                csmsg("Founder for %s changed to \x02%s\x0F." %
                                      (channel, params))
                            del db, cur, r
                        ...
                        if not 'A' in client.modes and client.nick != c['owner']:
                            ...
                            if not r:
                                ...
                                csmsg("Successor for %s changed to \x02%s\x0F." %
                                      (channel, params))
                            del db, cur, r
                        ...
                        if not 'A' in client.modes and client.nick != c['owner']:
                            ...
                            csmsg("Password for %s changed to \x02%s\x0F." %
                                  (channel, params))

                    elif option == 'desc':
                        c['description'] = escape(params)
                        csmsg("Description for %s changed to \x02%s\x0F." %
                              (channel, params))

                    elif option == 'url':
                        c['url'] = escape(params)
                        csmsg("URL for %s changed to \x02%s\x0F" %
                              (channel, params))
                        ...
                        if not 'A' in client.modes and client.nick != c['owner']:
                            ...
                            csmsg("Email address for %s changed to \x02%s\x0F." %
                                  (channel, params))
                        ...
                        if params.lower() == 'off':
                            ...
                            csmsg("Entry message for %s changed to \x02%s\x0F." %
                                  (channel, params))

                    elif option.lower() in ['mlock', 'keeptopic', 'peace',
                                            'restricted', 'secureops',
                                            'topiclock']:
                        if params.lower(
                        ) == 'off' and not c[option] or params.lower(
                        ) == c[option]:
                            csmsg("%s is already \x02%s\x0F for %s." %
                                  (option.title(), params.upper(), channel))
                        else:
                            if params.lower() == 'off':
                                ...
                                if option.lower() == 'mlock':
                                    ...
                                    if not chan:
                                        csmsg("\x02%s\x0F isn't active at the moment. No modes appended."
                                              % chan)
                                    else:
                                        for mode, settings in chan.modes.items():
                                            ...
                                            # invites across channel deaths.
                                                ...
                                                c['modes:+%s' % mode] = ''
                                            elif type(settings) in [
                                                str, unicode, int, float
                                            ]:
                                                c['modes:+%s' %
                                                  mode] = str(settings)
                                ...
                                c[option] = 'on'
                            csmsg("%s for %s set to \x02%s\x0F." %
        ...
        if not args or len(args.split()) != 2:
            ...
            if (not 'R' in client.modes or client.nick !=
                c['owner']) and not client.oper:
                csmsg("Access denied.")
            elif not c.r:
                csmsg("\x02%s\x0F is not registered." % chan)
            elif not channel:
                csmsg("\x02%s\x0F is not in use." % chan)
            else:
                ops = c['operators']
                if what == 'set':
                    ...
                    for mode, settings in c['modes'].items():
                        if not mode[1:] in channel.modes:
                            ...
                            csmode(channel, mode, settings)
                    if modes:
                        csmsg("Enforced \x02%s\x0F on \x02%s\x0F." %
                              (', '.join(modes.keys()), channel.name))
                    else:
                        csmsg("Enforced modes.")
                else:
                    csmsg("Unkown option \x02%s\x0F." % what)

    elif cmd == 'sop':
        if not args or args.split()[1].lower() not in ['add', 'del', 'list',
                                                       'clear']:
            csmsg(
                "Syntax: \x02SOP \x1Fchannel\x0F \x02{ADD|DEL|LIST|CLEAR} [\x1Fnick\x0F\x02]\x0F")
            ...
            if not c.r: csmsg("%s isn't registered." % chan)
            elif ('R' not in client.modes or client.nick !=
                  c['owner']) and not client.oper:
                csmsg("Access denied.")
            else:
                if params[0].lower() in ['add', 'del']:
                    ...
                    if not r:
                        ...
                        csmsg("\x02%s\x0F added to %s SOP list." %
                              (nick, chan))
                    elif params[0].lower() == 'del':
                        ops = c['operators']
                        if nick not in ops or (nick in ops and ops[nick] !=
                                               'a'):
                            csmsg("\x02%s\x0F is not in the SOP list for %s." %
                                  (nick, chan))
                        else:
                            del c['operators:%s' % nick]
                            csmsg("Removed \x02%s\x0F from %s SOP list." %
                                  (nick, chan))
                    del db, cur, r
            ...
            if not c.r: csmsg("%s isn't registered." % chan)
                ...
                if params[0].lower() in ['add', 'del']:
                    ...
                    if not r:
                        ...
                        csmsg("\x02%s\x0F added to %s AOP list." %
                              (nick, chan))
                    elif params[0].lower() == 'del':
                        ops = c['operators']
                        if nick not in ops or (nick in ops and ops[nick] !=
                                               'o'):
                            csmsg("\x02%s\x0F is not in the AOP list for %s." %
                                  (nick, chan))
                        else:
                            del c['operators:%s' % nick]
                            csmsg("Removed \x02%s\x0F from %s AOP list." %
                                  (nick, chan))
                    del db, cur, r
                elif params[0].lower() == 'list':
            ...
            if not c.r: csmsg("%s isn't registered." % chan)
                ...
                if params[0].lower() in ['add', 'del']:
                    ...
                    if not r:
                        ...
                        csmsg("\x02%s\x0F added to %s HOP list." %
                              (nick, chan))
                    elif params[0].lower() == 'del':
                        ops = c['operators']
                        if nick not in ops or (nick in ops and ops[nick] != 'h'):
                            ...
                            del c['operators:%s' % nick]
                            csmsg("Removed \x02%s\x0F from %s HOP list." %
                                  (nick, chan))
                    del db, cur, r
                elif params[0].lower() == 'list':
        ...
        if not args or args.split()[1].lower() not in ['add', 'del', 'list',
                                                       'clear']:
            csmsg(
                "Syntax: \x02VOP \x1Fchannel\ x0F\x02{ADD|DEL|LIST|CLEAR} [\x1Fnick\x0F\x02]\x0F")
            ...
            if not c.r: csmsg("%s isn't registered." % chan)
                ...
                if params[0].lower() in ['add', 'del']:
                    ...
                    if not r:
                        ...
                        csmsg("\x02%s\x0F added to %s VOP list." %
                              (nick, chan))
                    elif params[0].lower() == 'del':
                        ops = c['operators']
                        if nick not in ops or (nick in ops and ops[nick] !=
                                               'v'):
                            csmsg("\x02%s\x0F is not in the VOP list for %s." %
                                  (nick, chan))
                        else:
                            del c['operators:%s' % nick]
                            csmsg("Removed \x02%s\x0F from %s VOP list." %
                                  (nick, chan))
                    del db, cur, r
                elif params[0].lower() == 'list':
        ...
        if not args or len(args.split(' ', 1)) != 2:
            ...
            if not c.r: csmsg("\x02%s\x0F isn't registered." % chan)
                ...
                if not client.oper and 'R' not in client.modes or client.nick != c['owner'] and client.nick not in o \
                    ...
                    if m in b:
                        csmsg("\x02%s\x0F is already banned from %s." %
                              (mask, chan))
                    else:
        ...
        if not args or len(args.split(' ', 1)) != 2:
            csmsg(
                "Syntax: \x02/CHANSERV UNBAN \x1F#channel\x0F \x02\x1Fmask\x0F")
        else:
            chan, mask = args.split(' ', 1)
            chan = escape(chan)
            mask = escape(mask)
        ...
        if not args or len(args.split(' ', 1)) != 2:
            ...
            elif (client.nick != c['owner'] or 'R' not in
                  client.modes) and not client.oper:
                csmsg("Access denied.")
            else:
                if what == 'modes':
                    ...
                    if 'h' in channel.modes and len(channel.modes['h']) > 0:
                        ...
                        csmsg("Cleared Half-Operators list on \x02%s\x0F" %
                              chan)
                elif what == 'voices':
                    ...
                    csmsg("Unknown setting \x02%s\x0F." % what)
        ...
        if not args or len(args.split(' ', 1)) != 2:
            ...
            if not c.r:
                ...
                csmsg("%s is already protected in \x02%s\x0F." % (nick, chan))
            else:
                db = cache['db']
                cur = db.cursor()
                cur.execute("SELECT * FROM %s WHERE nick='%s'" %
                            (NS_TABLE, escape(nick)))
                r = cur.fetchone()
        ...
        if not args or len(args.split(' ', 1)) != 2:
            ...
            if not c.r:
                ...
                csmsg("%s isn't in the list of protected users for \x02%s\x0F." %
                      (nick, chan))
            else:
                del c['protected:%s' % nick]
                csmsg("Removed %s from the list of protected users for \x02%s\x0F."
                      % (nick, chan))
        ...
        if not args: csmsg("Syntax: \x02/CHANSERV INVITE \x1Fchannel\x0F")
            ...
            if not c.r or not channel:
                csmsg("Channel \x02%s\x0F doesn't exist")
            elif 'i' not in channel.modes:
                csmsg("\x02%s\x0F is not +i.")
            else:
                o = c['operators']
        ...
        if not args or not ' ' in args or len(args.split(' ', 2)) != 3:
            csmsg(
                "Usage: \x02/CHANSERV KICK \x1Fchannel\x0F \x02\x1Fnick\x0F \x02\x1Freason\x0F")
        else:
                    ...
                    if 'R' not in client.modes:
                        ...
                        for i in channel.clients:
                            if i.nick == nick:
                                ...
                                break
                        if not user:
                            csmsg("\x02%s\x0F is not currently on channel %s" %
                                  (nick, channel_name))
                        else:
                            if 'Q' in user.modes:
                                csmsg("Cannot kick %s. (+Q)" % nick)
                            else:
                                for op_list in channel.ops:
        ...
        if not args or len(args.split(' ', 1)) < 2:
            csmsg(
                "Usage: \x02/CHANSERV TOPIC \x1Fchannel\x0F \x02\x1Ftopic\x0F")
        else:
            chan = args.split()[0]
            ...
            if not c.r: csmsg("%s isn't registered." % chan)
                ...
                if not client.nick in ops and client.nick != c['owner'] and client.nick != c['successor']:
                    ...
                    if topic != c['topic']:
                        ...
                        c['topic_time'] = str(time.time())[:10]
                        if not channel:
                            csmsg("Stored topic for %s changed to \x02%s\x0F." %
                                  (chan, topic))
                    if channel and channel.topic != topic:
                        channel.topic = topic
                        channel.topic_time = str(time.time())[:10]
                        client.broadcast(channel.name, ':%s TOPIC %s :%s' %
                                         (CS_IDENT, chan, topic))
                        csmsg("Topic of %s changed to \x02%s\x0F" %
                              (chan, topic))
            del c
        ...
        if not args: csmsg("Usage \x02/CHANSERV LIST \x1Fpattern\x0F")
            ...
            if not client.oper or 'R' not in client.modes:
                ...
                db = cache['db']
                c = db.cursor()
                c.execute("SELECT * FROM %s WHERE channel LIKE '%s'" %
                          (TABLE, args))
                t = c.fetchall()
                csmsg_list(t)
                del db, c, t
        ...
        if not args: csmsg("Usage: \x02/CHANSERV INFO \x1FCHANNEL\x0F")
            ...
            if not c.r: csmsg("\x02%s\x0F isn't registered." % args)
                ...
                ops = c['operators']
                if channel:
                    csmsg(" \x02%s\x0F is active with %i client(s)" %
                          (args, len(channel.clients)))
                else:
                ...
                if bans:
                    if client.oper or ('R' in client.modes and client.nick == c['owner']) \
                        ...
                        for index, (mask, setter) in enumerate(bans):
                            if index == 0: continue
                            mask = re_to_irc(mask)
                            csmsg(' ' * 17 + '\x02%s\x0F %s(%s)' %
                                  (mask, ' ' * int(l - len(mask)), setter))
            del c

    elif cmd == 'drop':
        if not args or not ' ' in args:
            csmsg(
                "Usage: \x02/CHANSERV DROP \x1Fchannel\x0F \x02\x1Fpassword\x0F")
        else:
            ...
            if not r: csmsg("\x02%s\x0F isn't registered." % channel_name)
                ...
                if client.oper or (r['password'] == password):
                    c.execute("DELETE FROM %s WHERE channel='%s'" % \
						(TABLE, escape(channel_name)))
                    db.commit()
        ...
        if not client.oper:
            ...
            csmsg(
                "Use \x02/CHANSERV HELP\x0F for a list of available commands.")
        else:
            db = cache['db']
            c = db.cursor()
