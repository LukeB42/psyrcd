			...
			if type(o) == dict:
				if v in o:
					...
					self.c.execute("UPDATE %s SET %s='%s' WHERE channel='%s'" % \
						(TABLE,k,v,self.channel))
					self.db.commit()
		elif key == 'channel': self.c.execute("DELETE FROM %s WHERE channel='%s'" % (TABLE,self.channel))
		else: self[key] = ''
...
def init_channel(client, channel):
	...
	if c.r:
		...
		db = cache['db']
		cur = db.cursor()
		cur.execute("SELECT * FROM %s WHERE nick='%s'" % (NS_TABLE,c['owner']))
		r = cur.fetchone()
		if not r:
			if c['successor']:
				cur.execute("SELECT * FROM %s WHERE nick='%s'" % (NS_TABLE,c['successor']))
				s = cur.fetchone()
				if s:
					c['owner'] = c['successor']
					c['successor'] = ''
				...
				if not 'A' in client.modes and client.nick != c['owner'] and client.nick != c['successor']:
					...
					if option == 'founder':
						if not 'A' in client.modes and client.nick != c['owner']:
							...
							db = cache['db']
							cur = db.cursor()
							cur.execute("SELECT * FROM %s WHERE nick='%s'" % (NS_TABLE,escape(params)))
							r = cur.fetchone()
...
#					del c['bans']
		...
		if not args or len(args.split(' ',1)) != 2:
			...
			if not c.r:
				...
				db = cache['db']
				cur = db.cursor()
				cur.execute("SELECT * FROM %s WHERE nick='%s'" % (NS_TABLE,escape(nick)))
				r = cur.fetchone()
		...
		if not args: csmsg("Usage \x02/CHANSERV LIST \x1Fpattern\x0F")
			...
			if not client.oper or 'R' not in client.modes:
				...
				db = cache['db']
				c = db.cursor()
				c.execute("SELECT * FROM %s WHERE channel LIKE '%s'" % (TABLE,args))
				t = c.fetchall()
				csmsg_list(t)
				del db,c,t
